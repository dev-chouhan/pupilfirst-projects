...
let createInitialsFromName = (name) => {
  ...
  let len = word.length;
  if(len > 1){
    opt = word[0].slice(0, 1).toUpperCase();
    opt = opt + word[len-1].slice(0, 1).toUpperCase();
  } else {
    opt = word[0].slice(0, 2).toUpperCase();
  }
...
// console.log(createInitialsFromName("John Wick"));
// console.log(createInitialsFromName("John Baby Hereos Nope"));
